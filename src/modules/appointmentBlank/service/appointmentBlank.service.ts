...
import { CreateTreatmentPlanGraph } from '../model/treatment/treatment_plan.model';
...
export class AppointmentBlankService extends BasicService<AppointmentBlank> {
    ...
    async create(
        ...
        this.sendNotificationAppointment({
            treatmentPlan: args.treatmentPlan,
    ...
    async sendNotificationAppointment(args: {
        treatmentPlan: CreateTreatmentPlanGraph;
        userId: ObjectId;
        doctorId: ObjectId;
    }) {
        const [doctor, user] = await Promise.all([
            this.doctorService.findOne({
                _id: args.doctorId,
            }),
            this.userService.findOne({ _id: args.userId }),
        ]);

        await this.notificationService.sendTreatmentCalendarNotification({
            treatmentEvents: [
                ...args.treatmentPlan.medical,
                ...args.treatmentPlan.proccess,
                ...args.treatmentPlan.recomendation,
            ],
            doctor,
            user,
        });
