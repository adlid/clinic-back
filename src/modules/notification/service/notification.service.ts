...
export class NotificationService extends BasicService<Notification> {
    ...
    async sendTreatmentCalendarNotification(
        args: sendTreatmentCalendarNotificationArgs,
    ) {
        const { treatmentEvents, user, doctor } = args;
        const eventName = `Лечение от ${doctor.fullName}`;
        const events: ICalEventData[] = treatmentEvents.map(
            ({ text, repeatingOptions }): ICalEventData => {
                return {
                    summary: eventName,
                    description: text,
                    start: new Date(),
        ...
        const calendar = ical({ name: eventName, events });
        await this.mailService.sendOneMailWithCalendarEvent({
            calendar,
            sender: this.senderMail,
            reciever: user.email,

            text: `Ваш курс лечения от доктора ${doctor.fullName}`,
            subject: `Ваш курс лечения`,
        });
    }
...
interface sendTreatmentCalendarNotificationArgs {
    treatmentEvents: { repeatingOptions: ICalRepeatingOptions; text: string }[];
    user: User;
    doctor: Doctor;
}
